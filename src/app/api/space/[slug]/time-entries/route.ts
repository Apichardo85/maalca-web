...
// Ponches de entrada/salida del equipo (gestión de equipo — workforce). Reservado a
// Owner/Manager en el backend (Staff recibe 403) — datos sensibles de horas/nómina.
export async function GET(
  ...
  const apiRes = await fetch(`${API}/api/affiliates/${affiliate.id}/time-entries${qs ? `?${qs}` : ''}`, {
