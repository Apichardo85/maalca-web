...
export async function PATCH(
  ...
  const apiRes = await fetch(
    `${API}/api/affiliates/${affiliate.id}/appointments/${appointmentId}?status=${encodeURIComponent(status)}`,
    {
      method: 'PATCH',
...
export async function DELETE(
  ...
  const apiRes = await fetch(`${API}/api/affiliates/${affiliate.id}/appointments/${appointmentId}`, {
    method: 'DELETE',
    headers: { Authorization: `Bearer ${token}`, 'X-Affiliate-Id': affiliate.id },
  });
