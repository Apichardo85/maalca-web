...
// Tareas asignadas a miembros del equipo (gestión de equipo — workforce).
export async function GET(
  ...
  const apiRes = await fetch(`${API}/api/affiliates/${affiliate.id}/tasks${qs ? `?${qs}` : ''}`, {
...
export async function POST(
  ...
  const apiRes = await fetch(`${API}/api/affiliates/${affiliate.id}/tasks`, {
    method: 'POST',
    headers: {
