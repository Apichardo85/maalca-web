...
export async function PUT(
  ...
  const apiRes = await fetch(`${API}/api/affiliates/${affiliate.id}/modifier-groups/${id}`, {
    method: 'PUT',
    headers: {
...
export async function DELETE(
  ...
  // El backend bloquea borrar un grupo en uso por productos (InvalidOperationException con
  // los nombres de los platos) — se propaga tal cual para que la UI muestre el motivo real.
  const data = await apiRes.json().catch(() => null);
  return NextResponse.json(data ?? {}, { status: apiRes.status });
}
