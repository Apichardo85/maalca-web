...
export default function GalleryClient({ images }: GalleryClientProps) {
  ...
  return (
    <>
          ...
          {categories.map(cat => (
            <button
              ...
              style={{
                ...
                transition:'all .15s',
                background: active === cat ? 'var(--p)' : 'var(--l2)',
                color: active === cat ? '#fff' : 'var(--tm)',
              }}
            >
              {cat}
            </button>
      ...
      {/* ── BENTO GALLERY GRID ───────────────────────────────── */}
      <section style={{ padding:'clamp(2.5rem,5vw,4rem) clamp(1.5rem,5vw,5rem)', background:'var(--bg)' }}>
        <div style={{
          ...
          {filtered.map((img, idx) => (
            ...
            >
              ...
              <style>{`
                div[data-gid="${img.id}"]:hover > div {
                  background: rgba(0,25,60,.35) !important;
                }
                div[data-gid="${img.id}"]:hover span { color: rgba(255,255,255,.9) !important; }
                div[data-gid="${img.id}"]:hover img { transform: scale(1.05); }
              `}</style>
            </div>
          ))}
        </div>
