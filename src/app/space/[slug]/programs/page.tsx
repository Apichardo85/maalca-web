...
// "Programas" -- rediseno backlog 2026-09-26 (ver CommunityProgram.cs y comentario en
// registry.ts). Solo businessType === 'community', mismo gating que /activities.
export default async function ProgramsPage({
  ...
  let initialPrograms: CommunityProgram[] = [];
  try {
    const res = await fetch(`${API}/api/affiliates/${affiliateId}/programs`, {
      headers: { Authorization: `Bearer ${token}`, 'X-Affiliate-Id': affiliateId },
      cache: 'no-store',
    });
    if (res.ok) initialPrograms = await res.json();
  } catch {
    // El listado arranca vacío en vez de tumbar la página entera si el backend no responde.
  }

  return <ProgramsContent slug={slug} initialPrograms={initialPrograms} />;
}
